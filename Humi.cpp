...
Humi::Humi(char* nomenclature, bool is_analog, bool is_connected, bool is_low_power, uint8_t pin_read, uint8_t pin_power):Sensor(nomenclature, is_analog, is_connected, is_low_power, pin_read, pin_power){
  if (get_is_connected()){
...
void Humi::update_data()
{
  if (get_is_connected()) {
    ...
    if(get_is_analog()){
    	...
    	set_data((int)analogRead(get_pin_read()));
    }
    else {
